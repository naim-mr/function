...
let rec exp_to_apron (e,t,ext:expr typed) =
  ...
  | T_int_const ((Finite a as inf),(Finite b as sup)) -> 
     Texpr1.Cst (Coeff.Interval (Interval.of_mpqf (Intinf.to_mpqf inf) (Intinf.to_mpqf sup)))
  | T_int_const (INF,(Finite b as sup)) -> 
      Texpr1.Cst (Coeff.Interval (Interval.of_scalar (Scalar.of_infty (-1)) (Intinf.to_mpqf sup |> Scalar.of_mpqf )))
  | T_int_const ((Finite a as inf),INF) -> 
        Texpr1.Cst (Coeff.Interval (Interval.of_scalar  (Intinf.to_mpqf inf |> Scalar.of_mpqf ) (Scalar.of_infty 1)))
  ...
  | T_binary (o, e1, e2) -> (
      ...
      | _ -> failwith "not supported or not supposed to be supported")
