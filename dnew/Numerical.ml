...
module Numerical (N : NUMERICAL) (C : CONSTRAINT) : PARTITION = struct
  ...
  let rec assume ?(pow = 5.) b =
    ...
    let x =
      fst
        (List.hd
           (List.sort
              (fun (x1, o1) (x2, o2) ->
                if Z.compare x1.var_id x2.var_id = 0 then 1
                else if Z.compare x1.var_id x2.var_id <> 0 then -1
                else compare o1 o2)
              occ))
    in
    ...
    if 1 = Scalar.is_infty sup then (
      if -1 = Scalar.is_infty inf then (
        ...
        assert (
          not (isBot { constraints = C.negate c :: b.constraints; env; vars }));
      ...
      then (
        ...
        if Scalar.cmp inf (Scalar.of_float p2) > 0 then
          ...
          (* split on: 
            ...
            *)
          assert (
            not (isBot { constraints = c :: c1 :: b.constraints; env; vars }));
      ...
      let mid =
        ...
        else mulScalar sup (Scalar.of_int 2)
      in
      let e = Linexpr1.make env in
      Linexpr1.set_coeff e v (Coeff.s_of_int (-1));
  ...
  let bwdAssign_underapprox (t : t) ((x, typ, ext), e) =
    match x with
    | T_var x ->
  ...
  let filter b (e, t, ext) =
    ...
    if !Config.resilience && !Config.domain = "polyhedra" then
      let b2 = f (Box.manager_alloc ()) b (e, t, ext) in
      {
        constraints = b1.constraints @ b2.constraints;
        env = b1.env;
        vars = b1.vars;
      }
    else b1
