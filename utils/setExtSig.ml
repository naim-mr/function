...
module type OrderedType = Mapext.OrderedType
...
module type S = sig
  ...
  *)
  ...
  (** [partition p s] returns a pair of sets [(s1, s2)], where [s1] is the set
      of all the elements of [s] that satisfy the predicate [p], and [s2] is the
      set of all the elements of [s] that do not satisfy [p]. *)
  ...
  (** Same as {!Set.S.min_elt_opt}, but returns the largest element of the given
      set.
      @since 4.05 *)
  ...
  (** [iter2 f1 f2 f s1 s2] applies [f1] to the elements only in [s1], [f2] to
      the elements only in [s2], and [f] to the elements in both [s1] and [s2].
  ...
  val fold2 :
    ...
    'a ->
    'a
  (** [fold2 f1 f2 f s1 s2 acc] applies [f1] to the elements only in [s1], [f2]
      to the elements only in [s2], and [f] to the elements both in [s1] and
      [s2]. The elements are considered in increasing order. *)

  val for_all2 :
    (elt -> bool) -> (elt -> bool) -> (elt -> bool) -> t -> t -> bool
  (** [for_all2 f1 f2 f s1 s2] is true if [f1] is true on all the elements only
      in [s1], [f2] is true on all the elements only in [s2], and [f] is true on
      all the elements both in [s1] and [s2]. The elements are considered in
      increasing order. *)

  val exists2 :
    (elt -> bool) -> (elt -> bool) -> (elt -> bool) -> t -> t -> bool
  (** [exists2 f1 f2 f s1 s2] is true if [f1] is true on one element only in
      [s1], or [f2] is true on one element only in [s2], or [f] is true on one
      element both in [s1] and [s2]. The elements are considered in increasing
      order. *)

  val iter2_diff : (elt -> unit) -> (elt -> unit) -> t -> t -> unit
  (** [iter2_diff f1 f2 s1 s2] applies [f1] to the elements only in [s1] and
      [f2] to the elements only in [s2]. The elements both in [s1] and [s2] are
      ignored. The elements are considered in increasing order. It is equivalent
      to calling [iter2] with [f = ignore], but more efficient. *)

  val fold2_diff : (elt -> 'a -> 'a) -> (elt -> 'a -> 'a) -> t -> t -> 'a -> 'a
  (** [fold2_diff f1 f2 s1 s2] applies [f1] to the elements only in [s1] and
      [f2] to the elements only in [s2]. The elements both in [s1] and [s2] are
      ignored. The elements are considered in increasing order. It is equivalent
      to calling [fold2] with [f = fun v acc -> acc], but more efficient. *)

  val for_all2_diff : (elt -> bool) -> (elt -> bool) -> t -> t -> bool
  (** [for_all2_diff f1 f2 f s1 s2] is true if [f1] is true on all the elements
      only in [s1] and [f2] is true on all the elements only in [s2]. The
      elements both in [s1] and [s2] are ignored. The elements are considered in
      increasing order. It is equivalent to calling [for_all2] with
      [f = fun x -> true], but more efficient. *)

  val exists2_diff : (elt -> bool) -> (elt -> bool) -> t -> t -> bool
  (** [exists2_diff f1 f2 f s1 s2] is true if [f1] is true on one element only
      in [s1] or if [f2] is true on one element only in [s2]. The elements both
      in [s1] and [s2] are ignored. The elements are considered in increasing
      order. It is equivalent to calling [exists2] with [f = fun x -> false],
      but more efficient. *)
  ...
  (** [symb_diff_list s1 s2] returns the list of elements in [s1] and not [s2],
      and the list of elements in [s2] and not in [s1]. *)

  val add_sym_diff : t -> elt list * elt list -> t
  (** [add_sym_diff s2 d] returns [s1] assuming that [d = sym_diff_list s1 s2],
      i.e., it reconstructs a set given its symmetric difference with another
      set. *)

  val iter_slice : (elt -> unit) -> t -> elt -> elt -> unit
  (** [iter_slice f m k1 k2] is similar to [iter f m], but only calls [f] on
      elements greater or equal to [k1] and smaller or equal to [k2]. It is as
      if, outside this range, [f k] has no effect. The elements are considered
      in increasing order. *)

  val fold_slice : (elt -> 'a -> 'a) -> t -> elt -> elt -> 'a -> 'a
  (** [fold_slice f m k1 k2 a] is similar to [fold f m], but only calls [f] on
      elements greater or equal to [k1] and smaller or equal to [k2]. It is as
      if, outside this range, [f k x = x] and has no effect. The elements are
  ...
  (** [for_all_slice f m k1 k2 a] is similar to [for_all f m], but only calls
      [f] on elements greater or equal to [k1] and smaller or equal to [k2]. It
      is as if, outside this range, [f k = true] and has no effect. The elements
      are considered in increasing order. *)

  val exists_slice : (elt -> bool) -> t -> elt -> elt -> bool
  (** [exists_slice f m k1 k2 a] is similar to [exists f m], but only calls [f]
      on elements greater or equal to [k1] and smaller or equal to [k2]. It is
      as if, outside this range, [f k = false] and has no effect. The elements
      are considered in increasing order. *)
  ...
  val print :
    set_printer -> (out_channel -> elt -> unit) -> out_channel -> t -> unit
