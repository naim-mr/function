...
type 'a t = { set : 'a set; compare : 'a compare }
...
let rec cut compare k = function
  ...
  | Node { l = l1; v = k1; r = r1; h = h1 } ->
      ...
      if c < 0 then
        let l2, d2, r2 = cut compare k l1 in
        (l2, d2, Node { l = r2; v = k1; r = r1; h = h1 })
      else if c > 0 then
        let l2, d2, r2 = cut compare k r1 in
        (Node { l = l1; v = k1; r = l2; h = h1 }, d2, r2)
      else (l1, true, r1)
...
let print_gen o printer key ch s =
  if s = Empty then o ch printer.print_empty
  else
...
let fprint printer key ch l =
  print_gen (fun fmt s -> Format.fprintf fmt "%s@," s) printer key ch l
