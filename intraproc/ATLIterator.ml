...
(* type for atl properties, instantiated with bExp for atomic propositions *)
type atl_property = Typed_syntax.expr typed ATLProperty.generic_property
...
let rec print_atl_property fmt (property : atl_property) =
  ...
  | X (cp,p) -> Format.fprintf fmt "<%a>X{%a}" print_controllables cp print_atl_property p
  | F (cp,p) -> Format.fprintf fmt "<%a>F{%a}" print_controllables cp print_atl_property p
  | G (cp,p) -> Format.fprintf fmt "<%a>G{%a}" print_controllables cp print_atl_property p
  | U (cp,p1, p2) ->
      Format.fprintf fmt "<%a>U{%a}{%a}" print_controllables cp print_atl_property p1 print_atl_property
        p2
  | AND (p1, p2) ->
      Format.fprintf fmt "AND{%a}{%a}" print_atl_property p1 print_atl_property
        p2
  | OR (p1, p2) ->
      Format.fprintf fmt "OR{%a}{%a}" print_atl_property p1 print_atl_property
        p2
  | NOT p -> Format.fprintf fmt "NOT{%a}" print_atl_property p

  let controllable controllable_players e = 
    match e with
       | Typed_syntax.T_var v, typ, ext
         when String.starts_with ~prefix:"nondet_in" v.var_name ->
           (match controllable_players with 
           | Some IR 
           | Some R -> true 
           | _ -> false)
...
(*
   ...
   The augmented program can be checked for termination with the following atl: 'AF{exit: true}'
*)
...
module ATLIterator (D : RANKING_FUNCTION) : Semantics.SEMANTIC = struct
  (*
  ...
  *)
  let until (quantifier : quantifier) (fwdInv : label -> D.B.t)
      (program : program) (inv_keep : inv) (inv_reset : inv)  (controllable_players:controllable_players): inv =
      ...
      | T_stat ((blockLabel, ext), (stmt, _), nextBlock) ->
          ...
          let in_state =
            ...
            | T_add_var (l, Some e) | T_assign ((l, _), e) ->
                bwd_assign ~controllable:(controllable controllable_players e) ?domain:pre_dom out_state
  ...
  *)
  ...
  let global (quantifier : quantifier) (fwdInv : label -> D.B.t) 
      ?(use_sink_state = false) (program : program) (fixed_point : inv) (controllable_players:controllable_players): inv =
    let branch_join, bwd_assign, bwd_filter = abstract_transformer quantifier in
    ...
    let rec bwd (out : D.t) (b : block) : D.t =
      ...
      | T_stat ((blockLabel, ext), (stmt, _), nextBlock) ->
          ...
          let new_in =
            ...
            | T_add_var (l, Some e) | T_assign ((l, _), e) ->
                (* Printf.printf "in global ? %b \n" ((controllable controllable_players e));
                D.print !fmt (bwd_assign ~controllable:(controllable controllable_players e) ?domain:pre_dom out_state (((T_var l, l.var_typ, l.var_extent), e)));
                Printf.printf "then \n";
                D.print !fmt (D.mask current_in @@ bwd_assign ~controllable:(controllable controllable_players e) ?domain:pre_dom out_state @@ (((T_var l, l.var_typ, l.var_extent), e))); *)
                let controllable = (controllable controllable_players e) in 
                D.mask ~controllable:controllable current_in
                @@ bwd_assign ~controllable:controllable ?domain:pre_dom out_state
                @@ ((T_var l, l.var_typ, l.var_extent), e)
            | T_assert (b, _) | T_assume b ->
  ...
  *)
  let next (quantifier : quantifier) (program : program) (fp : inv)  (controllable_players:controllable_players): inv =
    let branch_join, bwd_assign, bwd_filter = abstract_transformer quantifier in
    ...
    let rec aux (b : block) (nextOuterState : D.t) () =
      ...
      | T_stat ((blockLabel, ext), (stmt, _), nextBlock) -> (
          ...
          | T_add_var (l, Some e) | T_assign ((l, _), e) ->
              let s =
                bwd_assign ~controllable:(controllable controllable_players e) nextBlockState ((T_var l, l.var_typ, l.var_extent), e)
              in
              addInv blockLabel s
  ...
  *)
  ...
  (*
    Recusively compute fixed-point for atl properties 
  *)
  let compute (program : program) (property : atl_property): inv =
    ...
    let print_inv property inv =
      if not !minimal then (
        Format.fprintf !fmt "Property: %a\n\n" print_atl_property property;
        printInv !fmt inv)
    in
    let rec inv (property : atl_property) : inv =
      let result =
        match property with
        | Atomic (b, None) -> atomic program b
        | Atomic (b, Some l) -> label_atomic program l b
        | X (cp,p) -> a_next program (inv p) cp
        | F (cp,p) -> a_until program atomic_true_inv (inv p) cp
        | G (cp,p) -> a_global program (inv p) cp
        | U (cp,p1, p2) -> a_until program (inv p1) (inv p2) cp
