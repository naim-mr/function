...
module ApronDomain (Param : Banal_apron_utils.APRON_PARAM) = struct
  ...
  let normalize_lincons_array (ar : Lincons1.earray) : Lincons1.earray =
    array_of_lincons_list
      (Lincons1.array_get_env ar)
      (list_of_lincons_array ar)
  ...
  let bound_var (a : t) (v : var) ((l, h) : Lin.I.t) : t =
    if not (Lin.B.is_finite l || Lin.B.is_finite h) then a
    else
      let env = Abstract1.env a in
  ...
  let shift_var_itv (a : t) (v : var) (i : Lin.I.t) : t =
    ...
    if Lin.I.equal i Lin.I.zero then a
    else
  ...
  let bwd_shift_var_itv (a : t) (v : var) ((l, h) as i : Lin.I.t) : t =
    if Lin.I.equal i Lin.I.zero then a
      ...
      let a1, a2 =
        (shift_var_itv a v (sl, sl), shift_var_itv a v (sh, sh))
      in
      Abstract1.meet man a1 a2

  (* backward version of V := [-oo;+oo] *)
  let bwd_forget_var (a : t) (v : var) : t =
    if Abstract1.is_variable_unconstrained man a (apron_of_var v) then a
    else Abstract1.bottom man (Abstract1.env a)
  ...
  let bwd_copy_var (a : t) (v : var) (w : var) : t =
    ...
    Lincons1.set_list c
      [(Coeff.s_of_int 1, v); (Coeff.s_of_int (-1), w)]
      None ;
    let a = Abstract1.meet_lincons_array man a ar in
    (* forget v *)
    Abstract1.forget_array man a [|v|] false

  (* first try to replace constraints in a with those in pool (or c), then
     remove the contraint c *)
  let remove_constraint (a : t) (pool : Lincons1.t list) (c : Lin.lincons) :
      t =
    let env = Abstract1.env a in
  ...
  (* (* alternate add_cone: for each generator that satisfy the constraint,
     make a new polyhedron by adding it, then intersect all these polyhedra
     DOES NOT WORK AS WELL *) let add_cone (a:t) (dir:Generator1.t list)
     (e:Lin.linexpr) : t = (* homogenous expression *) let e = Lin.A.set_cst
     Lin.I.zero e in let cones = List.fold_left (fun acc g -> match
     Generator1.get_typ g with | Generator1.RAY | Generator1.RAYMOD -> let
     env v = Lin.I.of_coeff (Generator1.get_coeff g (apron_of_var v)) in (*
     sign of scalar product *) let x = Lin.B.sign (fst (Lin.A.eval env e)) in
     if x >= 0 then (* new cone *) let gg = array_of_generator_list
     (Abstract1.env a) [g] in (Abstract1.add_ray_array man a gg)::acc else
     acc | _ -> acc ) [] dir in (* intersect *) if cones = [] then a else
     List.fold_left (Abstract1.meet man) (List.hd cones) (List.tl cones) *)
  ...
  module PtMap = Mapext.Make (struct
    type t = float * float

    let compare = compare
  end)

  module LineMap = Mapext.Make (struct
  ...
  let print_svg (ch : out_channel) ?(color = false) ?window
      ((w, h) : int * int) (a : t) ((x, y) : var * var) =
    (* text processing drawing; put a white frame and try not put two texts
    ...
    let text x y anchor col s =
      ...
      Printf.fprintf ch
        "<text x='%g' y='%g' fill='none' stroke='white' stroke-width='3px' \
         font-size='14px' font-weight='bold' font-family='%s' \
         text-anchor='%s'>%s</text>\n"
        x y svg_font anchor s ;
      Printf.fprintf ch
        "<text x='%g' y='%g' fill='%s' font-size='14px' font-weight='bold' \
         font-family='%s' text-anchor='%s'>%s</text>\n"
        x y
        (if color then col else "black")
        svg_font anchor s
    in
    ...
    let b =
      ...
      | None ->
          ...
          for i = 0 to Generator1.array_length gen - 1 do
            ...
            if Generator1.get_typ g <> Generator1.VERTEX then fact := 3.
            else
      ...
      | Some (x0, y0, x1, y1) ->
          max
            (max (abs_float x0) (abs_float y0))
            (max (abs_float x1) (abs_float y1))
    in
    ...
    let g =
      Array.to_list
        (Array.init (Generator1.array_length gen') (fun i ->
             get_coord
    ...
    let l_to_r =
      List.sort
        (fun (_, x, y) (_, x', y') ->
          if x = x' then compare y' y else compare x x' )
        g
    in
    let _, _, u = try List.hd l_to_r with _ -> (0., 0., 0.) in
    let up = List.filter (fun (_, _, y) -> y >= u) l_to_r
    and down = List.rev (List.filter (fun (_, _, y) -> y < u) l_to_r) in
    (* coordinate transform *)
    let xx x = float_of_int w *. (x -. x0) /. (x1 -. x0)
    and yy y = float_of_int h *. (y -. y1) /. (y0 -. y1) in
    (* output header *)
    Printf.fprintf ch "<svg width='%ipx' height='%ipx'>\n" w h ;
    (* output background *)
    Printf.fprintf ch
      "<rect fill='white' stroke='none' x='%i' y='%i' width='%i' \
       height='%i'/>\n"
      0 0 w h ;
    (* output polygon *)
    Printf.fprintf ch "<polygon fill='lime' points='" ;
    List.iter
      (fun (_, x, y) -> Printf.fprintf ch "%g,%g " (xx x) (yy y))
      (up @ down) ;
    Printf.fprintf ch "'/>\n" ;
    (* output coordinate system *)
    Printf.fprintf ch
      "<line x1='%i' y1='%f' x2='%i' y2='%g' stroke='gray' \
       stroke-width='1px'/>\n"
      0 (yy 0.) w (yy 0.) ;
    Printf.fprintf ch
      "<line x1='%g' y1='%i' x2='%g' y2='%i' stroke='gray' \
       stroke-width='1px'/>\n"
      (xx 0.) 0 (xx 0.) h ;
    ( if svg_grid then
      let c = ref scale in
    ...
    Array.iter
      (fun ((c, a, b), op) ->
        ...
        let p =
          List.filter
            (fun (x, y) -> x >= x0 && x <= x1 && y >= y0 && y <= y1)
            p
        in
        Printf.fprintf ch "<polyline points='" ;
        List.iter (fun (x, y) -> Printf.fprintf ch "%g,%g " (xx x) (yy y)) p ;
        Printf.fprintf ch
          "' stroke='blue' stroke-width='1px' fill='none'/>\n" ;
        (* constraint test *)
        if List.length p > 0 then
          let u, v = List.hd p in
          let u = (0.95 *. u) +. (0.05 *. (x0 +. x1) /. 2.)
          and v = yy v +. if v >= (y0 +. y1) /. 2. then 12. else -6. in
          let ll =
            Lin.A.set_var x (Lin.I.of_float a)
              (Lin.A.set_var y (Lin.I.of_float b)
                 (Lin.A.cst (Lin.I.of_float c)) )
          in
          text (xx u) v
            (if u < (x0 +. x1) /. 2. then "start" else "end")
            "midnightblue"
            (Lin.A.to_string ll ^ " " ^ Lincons1.string_of_typ op ^ " 0") )
      l ;
    Printf.fprintf ch
      "<polygon fill='none' stroke='%s' stroke-width='1pt' points='"
      (if color then "blue" else "black") ;
    List.iter
      (fun (_, x, y) -> Printf.fprintf ch "%g,%g " (xx x) (yy y))
      (up @ down) ;
    Printf.fprintf ch "'/>\n" ;
    ...
    for i = 0 to Generator1.array_length gen - 1 do
      let g = Generator1.array_get gen i in
      if Generator1.get_typ g <> Generator1.VERTEX then ()
      else
        let _, x, y = get_coord (Generator1.get_linexpr1 g) in
        (* vertex dot *)
        Printf.fprintf ch
          "<circle cx='%f' cy='%f' r='3' fill='%s' stroke='%s' \
           stroke-width='2px'/>\n"
          (xx x) (yy y) "white"
          (if color then "darkgreen" else "black") ;
        (* vertex coord *)
        text (xx x)
          (if y >= u then yy y -. 8. else yy y +. 14.)
          "middle" "darkgreen"
          (Printf.sprintf "(%g,%g)" (x +. 0.) (y +. 0.))
    done ;
    (* output footer *)
  ...
  (* LaTeX *)
  let print_latex o a =
    if Abstract1.is_bottom man a then output_string o "\\bottom"
    else if Abstract1.is_top man a then output_string o "\\top"
    else
      let cons = Abstract1.to_lincons_array man a in
      let first = ref true in
      let rec pp_coeff c v =
        match c with
        | Coeff.Scalar s ->
            if v <> "" && Scalar.sgn s = 0 then ()
              ...
              if Scalar.sgn s < 0 then
                if v <> "" && Scalar.equal_int s (-1) then
                  output_string o "-"
                else Printf.fprintf o "-%s" (Scalar.to_string (Scalar.neg s))
              else if !first then
                if v <> "" && Scalar.equal_int s 1 then ()
                else output_string o (Scalar.to_string s)
              else if v <> "" && Scalar.equal_int s 1 then
                output_string o " + "
              else Printf.fprintf o "+%s" (Scalar.to_string s) ;
  ...
  let bot a = Abstract1.bottom man (Abstract1.env a)

  let top a = Abstract1.top man (Abstract1.env a)

  let is_bot a = Abstract1.is_bottom man a

  let subseteq a b = Abstract1.is_leq man a b

  let print fmt a = Abstract1.print fmt a
  ...
  let fwd_assign a dst e =
    ...
    let assign v = Abstract1.assign_linexpr man a (apron_of_var v) ee None in
    ( ( match dst with
      | STRONG v ->
          if v.var_scope = T_VOLATILE || v.var_scope = T_INPUT then
            set_var_itv a v (Lin.A.eval (lin_env a) l)
          else assign v
      | WEAK vl ->
  ...
  let bwd_meet a b m =
    ...
    | WIDEN ->
        if subseteq a b then a
          ...
          let gens =
            List.fold_left
              (fun acc g -> if sat_generator b g then g :: acc else acc)
              [] (generator_list_of_abs a)
          in
          (* make abstract element from remaining generators *)
          let r =
            add_generators (Abstract1.bottom man (Abstract1.env a)) gens
          in
          if trace_bwd then
            Format.printf
              "** bwd lower widening@\n\
  ...
  (* helper for bwd_filter and non-invertible bwd_assign; cpool and rpool are
     constraint and ray pools to guide the operation *)
  let add_space post cpool rpool c =
    let env = Abstract1.env post in
    if trace_bwd then
      Format.printf "**** add_space %s@\n"
        (Lin.cons_to_string (Lin.neg_cons c)) ;
    (* intersect with constraint *)
    let cons = apron_of_lincons_ar env c in
    let post =
      bwd_trace "** meet_constraint"
        (Abstract1.meet_lincons_array man post)
        cons
    in
    (* (* equality *) let cc = Abstract1.of_lincons_array man env
       (apron_of_lincons_ar env (fst c,Lin.L_EQ)) in *)
    if is_bot post then
      ...
      (*if Abstract1.is_leq man post cc then (* saturate c => add cone *)
      ...
      let post =
        if
          Abstract1.is_eq man post
            (Abstract1.meet_lincons_array man coned cons)
        then coned
        else post
      in
      (* remove constraint *)
  ...
  let bwd_filter post1 post2 err e typ pre =
    ...
    if trace_bwd then
      Format.printf "** bwd_filter true %s@\n" (Lin.cons_to_string c1) ;
  ...
  let add_space_var post v l =
    ...
    (* extrude along v *)
    let prism =
      bwd_trace "prism" (Abstract1.forget_array man sec [|av|]) false
    in
    (* meet with post *)
  ...
  let bwd_assign post err dst e pre =
    ...
    let post =
      ...
      | STRONG v ->
          if trace_bwd then
            Format.printf "** bwd_assign %s := %s@\n" v.var_name
              (Lin.expr_to_string (i, m)) ;
          if Lin.VMap.is_empty m then
            ...
            (* invertible case *)
            let post =
              bwd_trace "bwd_shift_var" (bwd_shift_var_itv post v) i
            in
            let l = Lin.invert true (lin_env pre) (Lin.I.zero, m) v in
            if trace_bwd then
              Format.printf "**** inverted %s@\n" (Lin.expr_to_string l) ;
            let e = apron_of_lin env l in
            bwd_trace "assign_linexpr"
              (Abstract1.assign_linexpr man post (apron_of_var v) e)
              None )
          else
            ...
            if trace_bwd then
              Format.printf "** !! bwd_assign post %a@\n" print post ;
            (* backward forget *)
...
module PolkaDomain = ApronDomain (struct
  type lib = Polka.strict Polka.t

  let manager = Polka.manager_alloc_strict ()
end)

module OctagonDomain = ApronDomain (struct
  type lib = Oct.t
