      ...
      List.fold_left
        (fun acc g ->
          ...
          | Generator1.RAY | Generator1.RAYMOD ->
              ...
              if Linexpr0.cmp ea g.Generator1.generator0.Generator0.linexpr0 = 0
              then acc
              else
...
*)
  ...
  let print_svg (ch : out_channel) ?(color = false) ?window ((w, h) : int * int)
      (a : t) ((x, y) : var * var) =
    ...
    *)
    ...
    (* output background *)
    Printf.fprintf ch
      "<rect fill='white' stroke='none' x='%i' y='%i' width='%i' height='%i'/>\n"
      0 0 w h;
    (* output polygon *)
    ...
    Printf.fprintf ch
      "<line x1='%i' y1='%f' x2='%i' y2='%g' stroke='gray' stroke-width='1px'/>\n"
      0 (yy 0.) w (yy 0.);
    Printf.fprintf ch
      "<line x1='%g' y1='%i' x2='%g' y2='%i' stroke='gray' stroke-width='1px'/>\n"
      (xx 0.) 0 (xx 0.) h;
    (if svg_grid then
       let c = ref scale in
       while !c <= x1 do
    ...
    Array.iter
      (fun ((c, a, b), op) ->
        ...
        let p =
          List.filter (fun (x, y) -> x >= x0 && x <= x1 && y >= y0 && y <= y1) p
        in
        ...
        | Coeff.Scalar s ->
            if v <> "" && Scalar.sgn s = 0 then ()
              ...
              if Scalar.sgn s < 0 then
                if v <> "" && Scalar.equal_int s (-1) then output_string o "-"
              ...
              else if v <> "" && Scalar.equal_int s 1 then output_string o " + "
...
*)
    if is_bot post then
      ...
      (*if Abstract1.is_leq man post cc then
      (* saturate c => add cone *)
      ...
      let post =
        if
          Abstract1.is_eq man post (Abstract1.meet_lincons_array man coned cons)
        then coned
        else post
      in
      (* remove constraint *)
