...
let to_base x = match x with INF | MINF -> raise Int.Overflow | Finite x -> x
...
let sign = function Finite x -> Int.sign x | INF -> 1 | MINF -> -1
let equal = ( = )
...
let neg = function Finite x -> Finite (Int.neg x) | INF -> MINF | MINF -> INF
let abs = function Finite x -> Finite (Int.abs x) | INF | MINF -> INF
