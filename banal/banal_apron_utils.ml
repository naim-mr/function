...
module Make (Param : APRON_PARAM) (Itv : INTERVAL) = struct
  ...
  let blin_of_lin (vset : VSet.t) (le : Linexpr1.t) : Lin.linexpr =
    ...
    let vmap =
      ...
      |> Seq.map (fun (v, av) ->
             (v, Linexpr1.get_coeff le av |> Lin.I.of_coeff) )
      |> Lin.VMap.of_seq
    in
    (cst, vmap)
  ...
  let neg_lincons (d : Lincons1.t) : Lincons1.t =
    Lincons1.make
      (Lincons1.get_linexpr1 d |> neg_linexpr)
      (Lincons1.get_typ d)
  ...
  let normalize_lincons (d : Lincons1.t) : Lincons1.t list =
    ...
    | Lincons1.EQ ->
        ...
        [d; neg_lincons d]
    | DISEQ | EQMOD _ -> failwith (__LOC__ ^ ": unexpected lincons kind")
    | _ -> [d]
  ...
  let safe_abs_of_lincons_array env (l : Lincons1.earray) =
    (* Workaround a bug in APRON which causes a segfault when a a constraint
       of the form `lin_expr +- inf >= 0` is converted into an abstract
       value.
    ...
    let i_inf =
      Coeff.(Interval {inf= Scalar.of_infty 1; sup= Scalar.of_infty 1})
    in
    ...
    for i = 0 to Lincons1.array_length l - 1 do
      ...
      (* ensure that all the coefficients of the variables are not
         infinite *)
      let all_finite =
        Array.to_seq (fst (Environment.vars env))
      ...
      if Coeff.equal s_inf cst || Coeff.equal i_inf cst then
        (* inf >= 0 is always true, replace it with 0 >= 0 *)
        Lincons1.array_set l i
          (Lincons1.make (Linexpr1.make env) Lincons1.SUPEQ) ;
      if Coeff.equal s_minf cst || Coeff.equal i_minf cst then (
  ...
  let neg_generator (g : Generator1.t) : Generator1.t =
    Generator1.make
      (Generator1.get_linexpr1 g |> neg_linexpr)
      (Generator1.get_typ g)

  (* normalize generators. Vertexs and rays are left unmodified, lines are
     converted into a pair of rays (thus the output is a list) *)
  ...
  let ray_seq_of_abs (a : t) : Generator1.t Seq.t =
    generator_seq_of_abs a
    |> Seq.filter (fun g ->
           Generator1.get_typ g = RAY || Generator1.get_typ g = RAYMOD )
        ...
        (fun a g ->
          let vals =
            Array.init (Array.length vars) (fun i ->
                ...
                le )
          in
          (* Note: use `assign_linexpr` as `of_box` does not work with
             fractionary coordinates. *)
          let vtx =
            Abstract1.assign_linexpr_array man (Abstract1.top man env) vars
              vals None
          in
  ...
  let abs_of_generators_list (env : Environment.t) (g : Generator1.t list) :
      t =
    add_generators (Abstract1.bottom man env) g

  (* try to remove a constraint. returns `Some a` if the contraint was
     removed, `None` otherwise *)
  let remove_lincons (c : Lincons1.t) (a : t) : t option =
    let lincons_list, found =
      lincons_seq_of_abs a
      |> Seq.fold_left
           (fun (acc_lst, found) cc ->
             if c = cc then (acc_lst, true) else (cc :: acc_lst, found) )
           ([], false)
    in
    if found then Some (abs_of_lincons_list (Abstract1.env a) lincons_list)
    else None
  ...
  let remove_lincons_and_harmonize (a : t) (pool : Lincons1.t list)
      (c : Lin.lincons) : t =
    let env = Abstract1.env a in
  ...
  let gen_saturating_cons cons vset a =
    ...
    |> Seq.fold_left
         (fun acc g ->
           let env =
            fun v -> Generator1.get_coeff g (apron_of_var v) |> Itv.of_coeff
           in
           if
             (Generator1.get_typ g = VERTEX && Lin.A.eval env le = Itv.zero)
             || Generator1.get_typ g = RAY
                && Lin.A.eval env le_no_cst = Itv.zero
           then g :: acc
           else acc )
         []
  ...
  (* check if a lincons is of the form `v>=c` or `v<=c`. If it is, returns
     `Some (sign, v)` where sign is `true` if `>=` or `false` if `<=`. If the
     lincons has a different form, returns `None`. *)
  let lincons_single_var (vset : VSet.t) (lincons : Lincons1.t) :
      (bool * var) option =
    let lb =
      VSet.fold
        (fun v acc ->
          if Coeff.equal_int (Lincons1.get_coeff lincons (apron_of_var v)) 1
          then v :: acc
          else acc )
        vset []
    in
    let ub =
      VSet.fold
        (fun v acc ->
          if
            Coeff.equal_int (Lincons1.get_coeff lincons (apron_of_var v)) (-1)
          then v :: acc
          else acc )
        vset []
    in
  ...
  let int_var_bnds (var : var) : (Int.t * Int.t) option =
    ...
    in
    match (inf, sup) with
    | Finite var_inf, Finite var_sup ->
        Some (var_inf, Int.(var_sup - var_inf + one))
    | _ -> None

  (* compute a sequence a quadrants containing the poly *)
  let get_quadrants (vset : VSet.t) (a : t) : var Seq.t * Int.t VMap.t Seq.t
      =
    if Abstract1.is_bottom man a then (Seq.empty, Seq.empty)
    else
      let unbound, bound =
        VSet.to_seq vset
        |> Seq.filter (fun v ->
               match v.var_typ with A_int _ | A_BOOL -> true | _ -> false )
        |> Seq.map (fun v ->
               match int_var_bnds v with
               | None -> (v, Seq.return Int.zero)
               | Some (var_inf, var_len) -> (
                   let inf, sup =
                     Abstract1.bound_variable man a (apron_of_var v)
                   ...
                   | Finite inf, Finite sup ->
                       ...
                       if Int.(sup_id - inf_id > of_int 2) then (v, Seq.empty)
                       else
                         ( v
                         , Seq.ints 0
      ...
      let rec doit seq vars =
        match Seq.uncons vars with
        | None -> seq
        | Some ((v, bnd), next) ->
            let seq =
              Seq.flat_map
                (fun idx -> Seq.map (fun id -> VMap.add v id idx) bnd)
                seq
            in
            doit seq next
      in
  ...
  let get_directions (a : t) : Linexpr1.t list =
    ...
    |> Seq.map (fun lc ->
           Linexpr1.set_cst lc (Coeff.s_of_int 0) ;
           lc )
    |> List.of_seq
  ...
  let refine_singleton vset post1 post2 =
    let get_singleton post =
      ...
      |> Seq.fold_left
           (fun acc lc ->
             if Lincons1.get_typ lc <> EQ then acc
             else
    ...
    if
      VMap.exists2o
        ...
        (fun v c1 c2 ->
          ...
          | A_int _ ->
              let c1 =
                Banal_itv_rat.of_coeff c1 |> Banal_itv_rat.to_int_exact_opt
                |> Option.get
              in
              let c2 =
                Banal_itv_rat.of_coeff c2 |> Banal_itv_rat.to_int_exact_opt
                |> Option.get
              in
              Int.(abs (c2 - c1) = one)
          | _ -> false )
    ...
    then
      ...
      if
        List.for_all (fun r1 -> List.exists (fun r2 -> r1 = r2) rays2) rays1
        && List.for_all
             (fun r2 -> List.exists (fun r1 -> r1 = r2) rays1)
             rays2
  ...
  let refine_behind_cons vset post1 post2 =
    ...
    |> Seq.find_map (fun cons ->
           ...
           let nz_real =
             Array.exists
               (fun v -> Lincons1.get_coeff cons v |> Coeff.is_zero |> not)
               real_v
           in
           let nint_int =
             Array.for_all
               (fun v ->
                 Lincons1.get_coeff cons v |> Banal_itv_rat.of_coeff
                 |> Banal_itv_rat.to_int_exact_opt |> Option.is_none )
               int_v
           in
           let nint_cst =
             Lincons1.get_cst cons |> Banal_itv_rat.of_coeff
             |> Banal_itv_rat.to_int_exact_opt |> Option.is_none
           in
           if nz_real || nint_int || nint_cst then None
           else
             let () = assert (Lincons1.get_typ cons = SUPEQ) in
             (* find c' (note: c' here has opposite polarity compared to the
                thm) *)
             let lin_inc = Lincons1.get_linexpr1 cons |> Linexpr1.copy in
             Linexpr1.set_cst lin_inc
               (Interval
                  ( Linexpr1.get_cst lin_inc |> Banal_itv_rat.of_coeff
                  |> Banal_itv_rat.add Banal_itv_rat.one
                  |> Banal_itv_rat.to_apron ) ) ;
             ...
             if Abstract1.is_bottom man post2_cons_inc then None
             else
               (* find the generators of post1 and post2 saturating
                  respectively c and c'. Keep only the common rays *)
               let post1_sat = gen_saturating_cons cons vset post1 in
               let post2_sat =
                 gen_saturating_cons cons_inc vset post2_cons_inc
                 |> List.filter (fun g1 ->
                        Generator1.get_typ g1 = VERTEX
                        || List.exists (fun g2 -> g1 = g2) post1_sat )
               in
               let post1_sat =
                 List.filter
                   (fun g1 ->
                     Generator1.get_typ g1 = VERTEX
                     || List.exists (fun g2 -> g1 = g2) post2_sat )
                   post1_sat
               in
               let () = assert (List.length post1_sat > 0) in
               if List.length post2_sat = 0 then None
               else
                 (* p1_nc *)
                 let post1_no_cons =
                   remove_lincons cons post1 |> Option.get
                 in
                 (* pm *)
                 let middle_poly =
                   add_generators
                 ...
                 let rhs =
                   match remove_lincons top_cons middle_poly with
                   | None ->
                       Abstract1.meet_array man
                         [|post1_no_cons; middle_poly; post2_cons_inc|]
                   | Some middle_poly_no_top ->
                       Abstract1.meet_array man
                         [|post1_no_cons; middle_poly_no_top; post2_cons_inc|]
                 in
                 ...
                 if
                   List.exists
                     (fun g1 -> not (List.exists (fun g2 -> g1 = g2) ray_rhs))
                     ray_post1
                 then None
                 else
