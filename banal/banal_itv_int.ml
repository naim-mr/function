...
let add ((l1, h1) : t) ((l2, h2) : t) : t = (B.add l1 l2, B.add h1 h2)
let sub ((l1, h1) : t) ((l2, h2) : t) : t = (B.sub l1 h2, B.sub h1 l2)
...
let rem ((l1, h1) : t) (i2 : t) : t bot * bool =
  ...
  then (Nb (l1, h1), false)
    ...
    is_singleton (l1, h1) && is_singleton i2
  then
    match (l1, l2) with
    | Finite a, Finite b ->
        let r = Finite (I.rem a b) in
        (Nb (r, r), false)
...
let modular (i : t) (l : I.t) (h : I.t) =
  ...
  | Finite a, Finite b ->
      ...
      if not (I.equal (I.fdiv a w) (I.fdiv b w)) then
        (Finite l, Finite h (* overlap *))
...
let magnitude ((l, h) : t) : B.t = B.max (B.abs l) (B.abs h)
...
let filter_neq ((l1, h1) as i1 : t) ((l2, h2) as i2 : t) : (t * t) bot =
  ...
  | false, true ->
      if B.equal l1 l2 then Nb ((B.succ l1, h1), i2)
      else if B.equal l2 h2 then Nb ((l1, B.pred h1), i2)
      else Nb (i1, i2)
