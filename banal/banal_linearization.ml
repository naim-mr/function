...
module Make (I : INTERVAL) = struct
  ...
  let type_itv (t : typ) : I.t =
    match t with
    | A_int (i, s) ->
        let l, h = int_type_set i s in
        I.of_intinf l h
    | A_float f ->
        let l, h = float_type_set f in
        I.of_floats l h
    | A_BOOL -> I.zero_one
  ...
  let cast_expr (env : env) (src : typ) (dst : typ) (l : linexpr) : linexpr =
    ...
    | A_float _, (A_int _ | A_BOOL) ->
        ...
        let err =
          ...
          if B.sign rl >= 0 then I.minus_one_zero
          else if B.sign rh <= 0 then I.zero_one
          else I.minus_one_one
        in
        A.add_cst l err
    (* float -> float (rounding) *)
    | A_float A_REAL, A_float A_DOUBLE -> round_float D.min_denormal D.ulp l
    ...
    | A_int _, A_float A_DOUBLE ->
        if B.leq (I.magnitude (A.eval env l)) (B.of_float_down D.max_exact) then
          l (* exact *)
        else round_float D.min_denormal D.ulp l
    | A_int _, A_float A_FLOAT ->
        if B.leq (I.magnitude (A.eval env l)) (B.of_float_down S.max_exact) then
          l (* exact *)
  ...
  let rec lin_expr (env : env) ((e, t, x) : expr typed) : linexpr =
    ...
    | T_binary (op, e1, e2) -> (
        ...
        | A_MODULO ->
            ...
            (* modulo by zero *)
            if B.sign m2 = 0 then A.top
            else if
              (* first arg fits in modulo interval *)
              B.gt m1 (B.neg m2) && B.lt h1 m2
            then l1
            else
              (* does not fit => whole modulo interval *)
              let r = B.sub_up h2 B.one in
              if B.sign m1 >= 0 then A.cst (B.zero, r)
              else if B.sign h1 <= 0 then A.cst (r, B.zero)
              else A.cst (B.neg r, r)
  ...
  let is_integer (_, t, _) =
    match t with A_int _ | A_BOOL -> true | A_float _ -> false
  ...
  *)
  ...
  let split_cons ((l, op) : lincons) : lincons * lincons =
    ...
    | L_NEQ_INT ->
        ((A.sub_cst l I.one, L_GEQ_INT), (A.sub_cst (A.neg l) I.one, L_GEQ_INT))
    | _ -> invalid_arg "split_cons: not an equality or disequality constraint"

  let project_expr (l : linexpr) (vl : var list) : linexpr = A.project l vl
