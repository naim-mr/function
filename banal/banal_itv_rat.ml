...
let equal ((l1, h1) : t) ((l2, h2) : t) : bool = R.equal l1 l2 && R.equal h1 h2
let subseteq ((l1, h1) : t) ((l2, h2) : t) : bool = R.geq l1 l2 && R.leq h1 h2
let join ((l1, h1) : t) ((l2, h2) : t) : t = (R.min l1 l2, R.max h1 h2)
...
let contains ((l, h) : t) (x : R.t) : bool = R.leq l x && R.leq x h
let is_bounded ((l, h) : t) = R.is_real l && R.is_real h
let is_singleton ((l, h) : t) : bool = R.is_real l && R.equal l h

(************************************************************************)
(* FORWARD OVER-APPROXIMATED ARITHMETICS *)
(************************************************************************)

let neg ((l, h) : t) : t = (R.neg h, R.neg l)

let abs ((l, h) : t) : t =
  if contains (l, h) R.zero then (R.zero, R.max (R.abs l) (R.abs h))
  else hull (R.abs l) (R.abs h)

let add ((l1, h1) : t) ((l2, h2) : t) : t = (R.add l1 l2, R.add h1 h2)
let sub ((l1, h1) : t) ((l2, h2) : t) : t = (R.sub l1 h2, R.sub h1 l2)

(* helper: oo * 0 = 0 when multiplying bounds *)
let bound_mul x y = if R.sign x = 0 || R.sign y = 0 then R.zero else R.mul x y
...
let range ((l, h) : t) : R.t = R.sub h l
let magnitude ((l, h) : t) : R.t = R.max (R.abs l) (R.abs h)
