...
and expr =
  ...
  | A_INPUT of string * (string ext * string ext) option 
...
let rec pp_expr fmt e =
  ...
  | A_INPUT (id , Some ((min,_),(max,_)))-> Format.fprintf fmt "input %s (%s,%s)" id min max
  | A_INPUT (id , None)-> Format.fprintf fmt "input %s" id
