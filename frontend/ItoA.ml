...
let typ_itoa ((t : IntermediateSyntax.typ), (_ : extent)) : AbstractSyntax.typ =
  match t with I_INT -> A_INT | I_PTR -> A_PTR
...
let rec exp_itoa (ctx : ctx) (* calling context *) (env : env) (* environment *)
    ...
    AbstractSyntax.exp * env * statements * statements =
  try
    match exp with
    ...
    | I_interval ((o1 : unaryOp option), i1, (o2 : unaryOp option), i2) ->
        let aux (o : unaryOp option) (i : string) =
          ...
          | _ ->
              raise
                (Invalid_argument
                   "exp_itoa: only constant bounds for interval are supported")
        in
        (A_arithmetic (A_interval (aux o1 i1, aux o2 i2), a), env, pre, post)
    | I_incr (e, ea) | I_decr (e, ea) -> (
        (* post-increment/decrement *)
        let aux (e : IntermediateSyntax.exp) =
          match e with
          | I_incr _ -> A_PLUS
          | I_decr _ -> A_MINUS
    ...
    | I_unary (o, e) -> (
        ...
        | A_arithmetic (e, ea) -> (
            ...
            | I_DEREF ->
                (A_arithmetic (A_aunary (A_DEREF, (e, ea)), ea), env, pre, post)
            | I_ADDR ->
                (A_arithmetic (A_aunary (A_ADDR, (e, ea)), ea), env, pre, post))
    ...
    | I_and (e1, e2) -> (
        ...
        | A_arithmetic (e1, a1), A_boolean (e2, a2) ->
            ...
            in
            let cmp2 =
              A_rbinary (A_GREATER_EQUAL, (e1, a1), annotate (A_const 1))
            in
            let bvar = A_bbinary (A_AND, (cmp1, a1), (cmp2, a1)) in
            ( A_boolean (A_bbinary (A_AND, (bvar, a1), (e2, a2)), a),
              env,
              pre,
              post )
        | A_boolean (e2, a2), A_arithmetic (e1, a1) ->
            let cmp1 =
              A_rbinary (A_LESS_EQUAL, (e1, a2), annotate (A_const (-1)))
            in
            let cmp2 =
              A_rbinary (A_GREATER_EQUAL, (e1, a2), annotate (A_const 1))
            in
            let bvar = A_bbinary (A_AND, (cmp1, a1), (cmp2, a1)) in
            ( A_boolean (A_bbinary (A_AND, (e2, a2), (bvar, a1)), a),
              env,
              pre,
              post )
        | A_arithmetic (e1, a1), A_arithmetic (e2, a2) ->
            ...
            let bvar = A_bbinary (A_AND, (cmp1, a1), (cmp2, a1)) in
            let cmp1 =
              A_rbinary (A_LESS_EQUAL, (e2, a2), annotate (A_const (-1)))
            in
            let cmp2 =
              A_rbinary (A_GREATER_EQUAL, (e2, a2), annotate (A_const 1))
            in
            let bvar2 = A_bbinary (A_AND, (cmp1, a1), (cmp2, a1)) in
            ( A_boolean (A_bbinary (A_AND, (bvar, a1), (bvar2, a2)), a),
              env,
              pre,
              post )
    ...
    | I_or (e1, e2) -> (
        ...
        | A_arithmetic (e1, a1), A_boolean (e2, a2) ->
            ...
            in
            let bvar = A_bbinary (A_AND, (cmp1, a1), (cmp2, a1)) in
            ( A_boolean (A_bbinary (A_OR, (bvar, a1), (e2, a2)), a),
              env,
              pre,
              post )
        | A_boolean (e2, a2), A_arithmetic (e1, a1) ->
            ...
            in
            let bvar = A_bbinary (A_AND, (cmp1, a1), (cmp2, a1)) in
            ( A_boolean (A_bbinary (A_OR, (e2, a2), (bvar, a1)), a),
              env,
              pre,
              post )
        | A_arithmetic (e1, a1), A_arithmetic (e2, a2) ->
            ...
            in
            let bvar2 = A_bbinary (A_AND, (cmp1, a1), (cmp2, a1)) in
            ( A_boolean (A_bbinary (A_OR, (bvar, a1), (bvar2, a2)), a),
              env,
              pre,
              post )
    ...
    | I_assign ((e1, ea1), o, e2) -> (
        ...
        | I_id x -> (
            ...
            let e2, env, pre, post = exp_itoa ctx env pre post e2 in
            let aux (o : IntermediateSyntax.assignOp) =
              match o with
              | I_PLUS_EQUAL -> A_PLUS
...
(** Converts a parsed property to it's AbstractSyntax version. In constrast to
    'property_itoa', this function takes the result of 'prog_itoa' and a main
    function as argument. This makes it possible to convert additional
    properties for a given program without having to construct the internal
    context and environment data structures after the program has already been
    converted. *)
...
let declarator_itoa ctx (* ctx *) gs (* var StringMap.t *) ls
  ...
  let vm =
    ...
    (* local scope: e.locals is updated *) else StringMap.add x v gs
...
let rec stmt_itoa (ctx : ctx) (* calling context *)
  ...
  | I_while (exp, stmt) -> (
      ...
      | A_arithmetic (A_INPUT, ea) ->
          ( pre @ [ (A_while (dummyId, (A_MAYBE_INP, ea), body), a) ] @ post,
            lenv )
      | A_arithmetic (e, ea) ->
          let lhs = (A_rbinary (A_LESS, (e, ea), (A_const 0, ea)), ea) in
          let rhs = (A_rbinary (A_GREATER, (e, ea), (A_const 0, ea)), ea) in
  ...
  | I_for (decl, exp1, exp2, stmt) -> (
      ...
      let stmt, lenv = stmt_itoa ctx lenv stmt in
      match (e1, e2) with
      | A_boolean e1, A_stmt | A_boolean e1, A_arithmetic (A_var _, _) ->
          ( stmts @ pre1
            @ [
                ( A_while
                    ( dummyId,
      ...
      | A_arithmetic (A_INPUT, ea1), A_arithmetic (A_var _, _) ->
          ( stmts @ pre1
            @ [
                ( A_while
                    ( dummyId,
