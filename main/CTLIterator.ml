...
module CTLIterator (D : RANKING_FUNCTION) : SEMANTIC = struct
  ...
  *)
  ...
  let fwdTaintMap : VarSet.t InvMap.t ref = ref InvMap.empty
    ...
    let rec bwd (out : D.t) (b : block) : D.t =
      ...
      | A_block (blockLabel, (stmt, _), nextBlock) ->
          ...
          let in_state =
            ...
            | A_return -> bot
            | A_assign ((l, _), (e, _)) ->
                bwd_assign ?domain:pre_dom out_state (l, e)
            ...
            | A_if ((b, ba), s1, s2) ->
                ...
                let in_if_filtered = bwd_filter ?domain:pre_dom in_if b in
                (* filter *)
                let in_else_filtered =
                  bwd_filter ?domain:pre_dom in_else (fst (negBExp (b, ba)))
                in
                (* filter *)
            ...
            | A_while (l, (b, ba), loop_body) ->
                ...
                let out_exit =
                  bwd_filter ?domain:pre_dom out_state (fst (negBExp (b, ba)))
                in
                  ...
                  if isLeqComp then (
                    if isLeqApprox then (
                      ...
                      if !tracebwd && not !minimal then
                        Format.fprintf !fmt "in'': %a\n" D.print in_state'';
                      let out_enter' =
                        bwd_filter ?domain:pre_dom (bwd in_state'' loop_body) b
                      in
                ...
                let initial_out_enter =
                  D.filter ?domain:pre_dom (bwd initial_in loop_body) b
                in
          ...
          in
          let in_state =
            if !refine then D.refine in_state (fwdInv blockLabel) else in_state
          in
  ...
  *)
  let global (quantifier : quantifier) (fwdInv : label -> D.B.t)
    ...
    let rec bwd (out : D.t) (b : block) : D.t =
      ...
      | A_block (blockLabel, (stmt, _), nextBlock) ->
          ...
          let new_in =
            ...
            | A_assign ((l, _), (e, _)) ->
                D.mask current_in (bwd_assign ?domain:pre_dom out_state (l, e))
            | A_assert (b, _) ->
            ...
            | A_if ((b, ba), s1, s2) ->
                ...
                let out_else =
                  bwd_filter ?domain:pre_dom (bwd out_state s2)
                    (fst (negBExp (b, ba)))
                in
    ...
    in
    let _ =
      bwd (if use_sink_state then zero else bot) program.mainFunction.funcBody
    in
