...
module GuaranteeIterator (D: RANKING_FUNCTION): SEMANTIC  =
struct
  type r = D.t
  module D = D
  module B = D.B
  ...
  let rec bwdStm ?(property =  StringMap.empty ) ?(domain) funcs env vars p s =
    ...
    | A_while (l,(b,ba),s) ->
      ...
      let rec aux i p2 n =
        ...
        if (D.isLeq COMPUTATIONAL i' i) then
          if (D.isLeq APPROXIMATION i' i) then
            ...
            if !tracebwd && not !minimal then
              Format.fprintf !fmt "i'': %a\n" D.print i'';
            aux i'' (D.filter (bwdBlk ~property:property funcs env vars i'' s) b) (n+1)
        else
      ...
      let p2 = D.filter (bwdBlk ~property:property funcs env vars i s) b in
      let p = aux i p2 1 in
      addBwdInv l p; p
    | A_call (f,ss) -> raise (Invalid_argument "bwdStm:A_call")
    | A_recall (f,ss) -> raise (Invalid_argument "bwdStm:A_recall")

  and bwdBlk ?(property= StringMap.empty) funcs env vars (p:D.t) (b:block) : D.t =
