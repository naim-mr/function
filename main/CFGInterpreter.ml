...
type 't abstract_transformer =
  ...
  bool ->
  (* is current node a loop head *)
  int ->
  (* iteration count, how many times has this node been processed *)
  't ->
  (* current value of abstract state for this node *)
  ('t * inst) list
  (** list of (abstract state, instruction) pairs for all predecessors
      (successor) nodes that have an incoming (outgoing) edge from (to) this
      node. *) ->
  bool * 't
(** type definition for abstact transformer.

    an abstract transformer is a function that transforms a given abstract w.r.t
    the semantics of a node in the control-flow-graph (cfg) *)
(* returns a boolean flag that indicates if a fixed point was 
...
let execute (type a)
    (* type of the abstract state that is computed for each node in 'cfg' *)
      (analysis_type : analysis_type)
      (* type of analysis, either forward or backward *)
    (abstract_transformer : a abstract_transformer)
      (* implementation of abstract transformer for this analysis *)
    (initial_value : a NodeMap.t)
      (* map that assigns an initial state to each node in the cfg *)
    (entry_node : node) (* entry point of the program analysis *) (cfg : cfg)
    (* control flow graph *) (loop_heads : bool NodeMap.t)
    (domSets : NodeSet.t NodeMap.t) : a NodeMap.t
    (* returns a map that assigns each node an abstract state that is the result of the analysis *)
    =
  ...
  let addPredecessorsToWorklist node =
    List.iter
      (fun arc ->
        let predecessor = arc.arc_src in
        if not inWorklist.(predecessor.node_id) then
          ...
          let unfinished =
            if NodeMap.find predecessor loop_heads then
              Queue.fold
                (fun status node ->
          ...
          if not unfinished then (
            Queue.add predecessor worklist;
            (* add predecessor to worklist *)
            inWorklist.(predecessor.node_id) <-
              true (* update inWorklist array*)))
      node.node_in
  in
  (* add all successors of 'node' to worklist if they are not already in it*)
  let addSuccessorsToWorklist node =
    List.iter
      (fun arc ->
        let successor = arc.arc_dst in
        if not inWorklist.(successor.node_id) then (
          Queue.add successor worklist;
  ...
  let rec aux (nodeMap : a NodeMap.t) : a NodeMap.t =
    ...
    if Queue.is_empty worklist then nodeMap (* Work list is empty => exit *)
      ...
      let fixedPoint, newState =
        abstract_transformer node isLoopHead processed.(node.node_id)
          currentState instStatePairs
      in
      ...
      if (not fixedPoint) || processed.(node.node_id) == 1 then
        updateWorklist node;
      if !trace then
        Format.fprintf !fmt
          "-fixedPoint: %b \n-new worklist: %a \n######### \n \n" fixedPoint
...
let backward_analysis (type a) (at : a abstract_transformer) =
  execute BACKWARD at
