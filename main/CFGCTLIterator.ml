...
module CFGCTLIterator (D : RANKING_FUNCTION) = struct
  ...
  let labeled_atomic bot (cfg : cfg) (label : string) (property : bool_expr) :
    ...
    let aux map arc =
      ...
      | CFG_label l ->
          if String.equal l label then NodeMap.add arc.arc_dst atomicState map
          else map
      | _ -> map
    in
  ...
  let transform_functions (quantifier : quantifier) =
    let join =
      D.join (if quantifier == UNIVERSAL then APPROXIMATION else COMPUTATIONAL)
    in
    ...
    let abstract_transformer
      ...
      if !CFGInterpreter.trace_states then
        Format.fprintf !fmt "old_state: \n%a \nnew_state: \n%a \n" D.print
          (D.compress current_state) D.print (D.compress newState);
  ...
  let compute (cfg : cfg) (main : Cfg.func) (loop_heads : bool NodeMap.t)
      (domSets : NodeSet.t NodeMap.t) (property : ctl_property) : inv =
    ...
    let rec inv (property : ctl_property) : inv =
      let result =
        ...
        | AG p ->
            ...
            backwardAnalysis (global UNIVERSAL fwdInv) init_value main.func_exit
  ...
  let analyze ?(precondition = CFG_bool_const true) (cfg : cfg)
    ...
    tree :=
      D.output_json
        (List.map Cfgfrontend.Conversion.of_var cfg.cfg_vars)
        programInvariant;
