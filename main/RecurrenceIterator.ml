...
module RecurrenceIterator (D : RANKING_FUNCTION) = struct
  ...
  let rec bwdStm ?(property = dummy_prop) ?domain funcs env vars p s =
    ...
    | A_label (l, _) ->
        let p =
          try D.reset p (fst (StringMap.find l (Semantics.get_bexp property)))
          with Not_found -> p
        in
        p (* TODO: is this OK? *)
        ...
        let rec aux m o =
          let rec auxaux i p2 n =
            let i' =
              D.reset ~mask:m
                (D.join APPROXIMATION p1 p2)
                (fst (StringMap.find "" (Semantics.get_bexp property)))
            in
          ...
          let p =
            D.join APPROXIMATION p1
              (D.filter (bwdBlk ~property funcs env vars p s) b)
          in
          ...
          if D.isLeq COMPUTATIONAL m' m && D.isLeq COMPUTATIONAL m m' then (
            let p = p in
            if !tracebwd && not !minimal then (
  ...
  and bwdBlk ?(property = dummy_prop) funcs env vars (p : r) (b : block) : r =
    ...
    | A_empty l ->
        ...
        let m =
          if !refine then D.meet APPROXIMATION (D.refine (D.top env vars) a) p
          else D.meet APPROXIMATION (D.top env vars) p
        in
        let p =
          D.reset ~mask:m p
            (fst (StringMap.find "" (Semantics.get_bexp property)))
