...
module TerminationIterator (D : RANKING_FUNCTION) : SEMANTIC = struct
  ...
  let rec bwdStm ?property ?domain funcs env vars p s tvl =
    match s with
    | T_label _ | T_print _-> p
    | T_RETURN -> D.zero ?domain env vars
    | T_BREAK -> failwith "nyi"
    | T_assign ((v, _), (exp,typ,_)) ->
        D.bwdAssign ?domain ~taint:true ~underapprox:false p (T_var v, exp)
    | T_assert ((exp,typ), _) -> D.filter ?domain p exp
    | T_assume ((exp,typ), _) -> D.filter ?domain p exp
    | T_if ((b, typ, ba), s1, s2) ->
        let uap = false in
        let p1 = bwdBlk funcs env vars p s1 in
        let p1 = D.filter ?domain ~underapprox:uap p1 b in
        let p2 = bwdBlk funcs env vars p s2 in
        let p2 =
          D.filter ?domain ~underapprox:uap p2 (fst (negBExp (b, ba)))
        in
        if !tracebwd && not !minimal then (
          Format.fprintf Format.std_formatter "if in p1: %a\n" D.print p1;
          Format.fprintf Format.std_formatter "p2: %a\n" D.print p2);
        let joinType = APPROXIMATION
        in
