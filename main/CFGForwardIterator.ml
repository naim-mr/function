...
module CFGForwardIterator (B: PARTITION) = struct
  ...
  let compute (cfg:cfg) (main:func) (loop_heads: bool NodeMap.t) (domSets : NodeSet.t NodeMap.t) : B.t NodeMap.t =
    ...
    let fwdInv = forward_analysis abstract_transformer initialState cfg.cfg_init_entry cfg loop_heads domSets in
    ...
    forward_analysis abstract_transformer initialState main.func_entry cfg loop_heads domSets
