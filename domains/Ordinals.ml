...
module OrdinalValued (F: FUNCTION) : FUNCTION =
struct
  ...
  let print fmt (f,ff) =
    let rec aux fmt (ff,i) =
      ...
      | [] -> ()
      | x::xs -> 
        if i > 1 then Format.fprintf fmt "%a (%a)⍵^%i + " aux (xs,i+1) F.print x i
        else Format.fprintf fmt "%a (%a)⍵ + " aux (xs,i+1) F.print x
    in Format.fprintf fmt "%a%a" aux (ff,1) F.print f
