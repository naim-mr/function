(** Constraint domain
    The signature [CONSTRAINT] define the domain of constraint that can be used to partition a Decision Tree
 *)
open Apron
...
module type CONSTRAINT = sig
  ...
  val compare : t -> t -> int 
  (** [compare t1 t2] compares the two constraints. 
      It returns -1 if c1 < c2; 0 if c1 = c2 and 1 if c1 > c2.
  *)
  ...
  (** [similar t1 t2] Tests if two constraints are similars e.g. differs only by a constant *)
...
module type AP_CONSTRAINT = CONSTRAINT with type env = lincons_env and type cons = Lincons1.t and type linexpr = Linexpr1.t