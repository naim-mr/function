...
let print_var fmt v = Format.fprintf fmt "%s" @@ unique_var_name v
...
let rec print_int_expr fmt e =
  ...
  | CFG_int_random -> Format.printf "?"
  | CFG_int_interval (i1, i2) ->
      Format.fprintf fmt "[%s,%s]" (Z.to_string i1) (Z.to_string i2)
...
let print_inst fmt i =
  ...
  | CFG_assert b -> Format.fprintf fmt "assert %a" print_bool_expr b
  | CFG_call f -> Format.fprintf fmt "call %s" f.func_name
  | CFG_label l -> Format.fprintf fmt "label %s" l
...
let print_cfg fmt p =
  let pp_var fmt v =
    Format.fprintf fmt "%s:%s" (unique_var_name v) (string_of_type v.var_type)
  in
