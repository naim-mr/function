...
let of_var (var : Cfg.var) : AbstractSyntax.var =
  {
    ...
    AbstractSyntax.varTyp = AbstractSyntax.A_INT;
    (* only int for now *)
  }
...
let of_binary_op (op : int_binary_op) : AbstractSyntax.aBinOp =
  match op with
  | AST_PLUS -> A_PLUS
  | AST_MINUS -> A_MINUS
...
let rec of_int_expr expr =
  ...
  | CFG_int_binary (op, e1, e2) ->
      A_abinary
        (of_binary_op op, annotate @@ of_int_expr e1, annotate @@ of_int_expr e2)
...
let rec of_bool_expr (expr : Cfg.bool_expr) : bExp =
  match expr with
  | CFG_bool_unary (AST_NOT, e) -> A_bunary (A_NOT, annotate @@ of_bool_expr e)
  ...
  | CFG_compare (AST_LESS_EQUAL, e1, e2) ->
      A_rbinary
        (A_LESS_EQUAL, annotate @@ of_int_expr e1, annotate @@ of_int_expr e2)
  | CFG_compare (AST_LESS, e1, e2) ->
      A_rbinary (A_LESS, annotate @@ of_int_expr e1, annotate @@ of_int_expr e2)
  | CFG_compare (AST_GREATER, e1, e2) ->
      A_rbinary
        (A_GREATER, annotate @@ of_int_expr e1, annotate @@ of_int_expr e2)
  | CFG_compare (AST_GREATER_EQUAL, e1, e2) ->
      A_rbinary
        (A_GREATER_EQUAL, annotate @@ of_int_expr e1, annotate @@ of_int_expr e2)
