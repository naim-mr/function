...
let const_node_map (cfg : cfg) value =
  List.fold_left
    (fun map node -> NodeMap.add node value map)
    NodeMap.empty cfg.cfg_nodes

let negate_bool_expr (bexpr : bool_expr) : bool_expr =
...
let valid_cfg (cfg : cfg) =
  ...
  in
  List.for_all validArc cfg.cfg_arcs

let max_arc_id cfg =
  List.fold_left
    (fun currentMax arc ->
      if arc.arc_id > currentMax then arc.arc_id else currentMax)
    0 cfg.cfg_arcs

let max_node_id cfg =
  List.fold_left
    (fun currentMax node ->
      if node.node_id > currentMax then node.node_id else currentMax)
    0 cfg.cfg_nodes
...
let insert_entry_exit_label (cfg : cfg) (func : func) =
  ...
  let newFunc =
    { func with func_exit = newExitNode; func_entry = newEntryNode }
  in
  let newCfgFuncs =
    List.map
      (fun f -> if f.func_id == newFunc.func_id then newFunc else f)
      cfg.cfg_funcs
  in
  let result =
    {
  ...
  let add_function_call_arc (cfg : cfg) (arc : arc) =
    ...
    | CFG_call f ->
        ...
        let new_arcs =
          func_dst_arc
          :: List.map
               (fun a -> if a.arc_id == arc.arc_id then src_func_arc else a)
               cfg.cfg_arcs
        in
        { cfg with cfg_arcs = new_arcs }
    | _ -> raise (Invalid_argument "Not a call arc")
  in
  let callArcs =
    List.filter
      (fun a -> match a.arc_inst with CFG_call f -> true | _ -> false)
      cfg.cfg_arcs
  in
...
let func_nodes (func : func) =
  let rec aux nodeMap (node : node) =
    if NodeMap.mem node nodeMap then nodeMap
    else
...
let called_functions (func : func) : func list =
  let isCallArc (a : arc) =
    match a.arc_inst with CFG_call _ -> true | _ -> false
  in
  let getFunc a =
...
let copy_function_nodes (cfg : cfg) (func : func) =
  ...
  let originalArcs =
    List.flatten @@ List.map (fun n -> n.node_out) originalNodes
  in

  (* construct map of old node ids to clones of nodes with new unique ids *)
  let addNode idMap n =
    IntMap.add n.node_id { n with node_id = nextNodeId () } idMap
  in
  ...
  List.iter
    (fun arc ->
      ...
      let newArc =
        { arc with arc_id = nextArcId (); arc_src = newSrc; arc_dst = newDst }
      in
  ...
  ( newCfg,
    IntMap.find func.func_entry.node_id idNodeMap,
    IntMap.find func.func_exit.node_id idNodeMap )
  ...
  let rec aux cfg =
    ...
    let cfg' =
      List.fold_left
        ...
        cfg cfg.cfg_arcs
    in
    if cfg == cfg' then cfg' else aux cfg'
    (* keep going until there are no more function calls that can be inlined *)
  in
  aux cfg
