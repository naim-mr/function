...
let rec int_expr (env : env) (expr : Abstract_syntax_tree.int_expr) :
  ...
  | AST_int_const (i, x) ->
      ...
      in
      (env, [], CFG_int_const v)
  | AST_int_random -> (env, [], CFG_int_random)
...
let func ?(is_main = false) (env : env) (f : fun_decl) : env =
  ...
  let env_get_main_property =
    if is_main then
      let env_get_main_property bexp =
        let _, _, cfg_bexp = bool_expr env4 bexp in
        cfg_bexp
      in
      env_get_main_property
...
let prog ((t, x) : toplevel list ext) (main : string) :
    cfg * (Abstract_syntax_tree.bool_expr -> Cfg.bool_expr) =
  (* initial environment *)
  arcs := [];
  nodes := [];
  let env_init =
