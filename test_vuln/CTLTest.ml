...
let test_ast file property ?(precondition = "true") ?(domain = POLYHEDRA)
    ?(joinbwd = 2) ?setup =
...
let test_cfg file property ?(precondition = "true") ?(domain = POLYHEDRA)
    ?(joinbwd = 2) ?setup =
  TestCommon.make_analyser
    [
      "-minimal";
...
let ctl_ast_testcases =
  "ctl_ast"
  >::: [
         test_ast ~joinbwd:4 "./tests/ctl/global_test_simple.c"
           "AG{AF{x <= -10}}" true;
         test_ast ~joinbwd:4 "./tests/ctl/global_test_simple.c"
           "EG{AF{x <= -10}}" true;
         test_ast ~joinbwd:4 "./tests/ctl/global_test_simple.c"
         ...
         test_ast ~joinbwd:4 "./tests/ctl/global_test_simple.c"
           "EG{EF{x <= -10}}" true;
         test_ast ~precondition:"x == y + 20" "./tests/ctl/until_test.c"
           "AU{x >= y}{x==y}" true;
         ...
         test_ast ~setup:[ "-ordinals"; "1" ] "./tests/sink.c" "AG{AF{x==0}}"
           true;
         test_ast ~precondition:"n > 0" "./tests/ctl/and_test.c"
           "AND{AG{AF{n==1}}}{AF{n==0}}" true;
         test_ast ~precondition:"n > 0" "./tests/ctl/and_test.c" "EG{AF{n==1}}"
           true;
         test_ast ~precondition:"n > 0" "./tests/ctl/and_test.c" "AG{EF{n==1}}"
           true;
         test_ast ~precondition:"n > 0" "./tests/ctl/and_test.c" "EG{EF{n==1}}"
           true;
         ...
         test_ast ~precondition:"2*x <= y+3" "./tests/ctl/existential_test1.c"
           "EF{r==1}" true;
         test_ast "./tests/ctl/existential_test1.c" "EF{r==1}" false;
         ...
         test_ast
           ...
           true;
         test_ast ~joinbwd:5 ~precondition:"x==2"
           "./tests/ctl/existential_test3.c" "EF{r==1}" true;
         test_ast ~precondition:"n > 0" "./tests/ctl/fin_ex.c" "EG{EF{n==1}}"
           true;
         test_ast ~precondition:"x > y" "./tests/ctl/until_existential.c"
           "EU{x >= y}{x == y}" true;
         test_ast ~precondition:"x > y" "./tests/ctl/until_existential.c"
           "EF{AG{x == y}}" true;
         test_ast "./tests/ctl/existential_test4.c" "EF{r==1}" true;
         test_ast ~precondition:"a!=1" "./tests/ctl/koskinen/acqrel_mod.c"
           "AG{OR{a!=1}{AF{r==1}}}" true;
         test_ast ~setup:[ "-ordinals"; "3" ] ~precondition:"A==0 && R==0"
...
let ctl_cfg_testcases =
  ...
  >::: [
         ...
         test_cfg ~precondition:"x==1" "./tests/ctl/next.c" "AX{AX{AX{x==0}}}"
           true;
         test_cfg "./tests/ctl/next.c" "AX{AX{x==0}}" false;
         test_cfg "./tests/ctl/existential_test1.c" "EF{r==1}" false;
         test_cfg ~precondition:"2*x <= y+3" "./tests/ctl/existential_test1.c"
           "EF{r==1}" true;
         test_cfg "./tests/ctl/existential_test1.c" "EF{r==1}" false;
         ...
         test_cfg "./tests/ctl/multi_branch_choice.c" "AF{OR{x==4}{x==-4}}" true;
         test_cfg "./tests/ctl/multi_branch_choice.c" "AND{EF{x==4}}{EF{x==-4}}"
           true;
