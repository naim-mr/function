...
let make_analyser common_setup ?(setup=[]) filename expected =
  ...
  assert_command ~exit_code:(Unix.WEXITED 0) ~foutput:(fun out ->
    let terminate = ref false in
    let ok_string = "\nAnalysis Result: TRUE" in
    let index = ref 0 in
    (* Format.fprintf Format.std_formatter "%s " filename; *)
    (* List.iter(fun s -> Format.fprintf Format.std_formatter "%s " s) common_setup; *)
    Stream.iter (fun c ->
  ...
  "timeout" (["120s"; "./main.exe"] @ common_setup @ setup @ [filename])
